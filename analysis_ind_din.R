...
#poisson w/ population as offset -- could also run with tests as offset 
#we add the per 100000 to make a per 100,000 rate offset
county_cases2$pre_post<-as.factor(county_cases2$pre_post)

summary(mod_p2<-glm(daily_count~treat1+ pre_post + offset(log(pop/100000)), family="poisson", data=county_cases2))
summary(mod_p2<-glm(daily_count~treat1+ pre_post, family="poisson", data=county_cases2))
summary(mod_offset<-glm(daily_count~treat1*pre_post + offset(log(pop/100000)), family="poisson", data=county_cases2))
confint(mod_offset)
...
as.numeric(2 * (logLik(mod_negbin) - logLik(mod_3c)), df = 1, lower.tail = FALSE)
lrtest(mod_3c, mod_negbin)

#need to estimate robust standard errors bc of violation of distribution assumption
#change code for my models
...
#then add the robust standard errors to model output (for the IRR's)
#need to substitute in my models...
